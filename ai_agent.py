...
def get_ai_daily_briefing(date_str: str = "2026-08-14") -> Dict[str, Any]:
    ...
    cases = data.get("court_summaries", [])
    ...
    for court in cases:
        for c in court.get("cases", []):
            ...
            if "bank of baroda" in title.lower() or "state bank of india" in title.lower():
                bank_cases.append(c)
    ...
    summary_text = (
        f"⚡ **JARVIS Morning Legal Briefing:** You have **{total_hearings} confirmed hearings** today across 6 Karur Courts.\n\n"
        f"🚨 **High Priority Action Items:**\n"
    )

    if warrant_cases:
        c = warrant_cases[0]
        summary_text += f"• **Warrant Matter:** Case `{c.get('case_number_formatted')}` in **{c.get('court_name')} ({c.get('court_room')}, Item #{c.get('item_number')})** - Presiding: {c.get('judge_name')}.\n"
    
    if urgent_cases:
        c = urgent_cases[0]
        summary_text += f"• **Urgent Interim Application / Execution:** Case `{c.get('case_number_formatted')}` in **{c.get('court_name')} ({c.get('court_room')}, Item #{c.get('item_number')})**.\n"

    summary_text += (
        f"\n🏛️ **Court Room Strategy:**\n"
        f"• **Principal Sub Court (Room 3 & Room 4):** 8 cases scheduled (Start here for Item #1 Commercial Suit COS/69/2024 before Judge Priyanga).\n"
        f"• **CJM Court (Room 8):** Item #4 (STC/1035/2023 - Palanisamy evidence cross-examination).\n"
        f"\n📲 **WhatsApp Status:** 14 client notices are pre-formatted and ready for 1-click dispatch."
    )
    ...
    return {
        ...
        "recommended_first_court": "Principal Sub Court, Room 3 (Item #1)"
    }
...
def query_agentic_ai(prompt: str) -> Dict[str, Any]:
    ...
    if "urgent" in p or "warrant" in p or "priority" in p or "mukkiyam" in p:
        ...
        for c in urgent_list:
            resp += f"• **{c.get('case_number_formatted')}** ({c.get('client_name')}): {c.get('court_name')} &bull; **{c.get('court_room')} (Item #{c.get('item_number')})** &bull; Stage: *{c.get('case_stage')}*\n"
        return {"success": True, "reply": resp}

    # 2. Bank / Institutional Matters
    if "bank" in p or "bob" in p or "sbi" in p:
        bank_list = [c for c in all_c if "bank" in (c.get("case_title") or "").lower()]
        resp = f"🏦 **Found {len(bank_list)} Institutional / Bank Matters:**\n\n"
        for c in bank_list:
            resp += f"• **{c.get('case_number_formatted')}** &bull; {c.get('case_title')}\n  🏛️ {c.get('court_name')} ({c.get('court_room')}, Item #{c.get('item_number')}) &bull; Date: **{c.get('next_hearing_date')}**\n"
        return {"success": True, "reply": resp}
    ...
    if "priyanga" in p or "sub judge" in p:
        ...
        for c in priyanga_list:
            resp += f"• **Item #{c.get('item_number')}** &bull; {c.get('case_number_formatted')} ({c.get('client_name')}) &bull; Stage: *{c.get('case_stage')}*\n"
        return {"success": True, "reply": resp}
    ...
    return {
        ...
        "reply": (
            ...
            f"• **Today's Hearings:** 14 Cases in Karur\n\n"
            f"💡 *You can ask me:*\n"
            f"1. *'Show urgent warrant cases today'*\n"
            f"2. *'List Bank of Baroda suits'*\n"
            f"3. *'Which court room should Advocate R. Anbaiya visit first?'*\n"
